...
if [ ! -e "$RUNDIR" ]; then
    mkdir -p "$RUNDIR/genetic" "$RUNDIR/ramses"
...
if [ ! -e "$RUNDIR/.genetic.complete" ]; then
    ...
    echo "[1/4] generate the inverted compact rank-729 level-14 IC"
    (
...
if [ ! -e "$RUNDIR/.ic_verify.complete" ]; then
    echo "[2/4] verify all six GRAFIC levels and the rank-729 mask"
    (
        TIMEFORMAT=$'real_seconds=%R\nuser_seconds=%U\nsystem_seconds=%S'
...
if [ ! -e "$RUNDIR/.ramses.complete" ]; then
    ...
    echo "[3/4] ingest the level-9 through level-14 hierarchy on 64 MPI ranks"
...
echo "[4/4] verify the AMR hierarchy and global particle IDs"
(
    TIMEFORMAT=$'real_seconds=%R\nuser_seconds=%U\nsystem_seconds=%S'
    time python3 "$RUNDIR/verify_compact_level14.py" \
        "$RUNDIR/genetic" "$RUNDIR/compact729.id" \
